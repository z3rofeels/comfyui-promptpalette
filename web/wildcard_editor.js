...
// Node creation can race a cold stylesheet load (especially in Desktop).
// Waiting prevents the unstyled, thousands-of-rows category tree from being
// measured once by Nodes 2.0 and written back as the node's permanent height.
const editorStylesReady = addStylesheet(CSS_HREF).catch((error) => {
  console.error("Prompt Palette: failed to load editor stylesheet", error);
  throw error;
});

// Every route below is registered by this node pack's own Python backend on
// PromptServer.instance.routes. ComfyUI's server mounts each such route
// twice -- once at its own path, once again under "/api" + that path -- so
// api.fetchApi() (which always resolves through the "/api"-prefixed alias)
// reaches the exact same handlers plain fetch() did, while additionally
// picking up the auth headers ComfyApi attaches to every request.
const API = {
  async list() {
    const r = await api.fetchApi("/prompt_palette/list");
    return (await r.json()).items || [];
  },
  async categories() {
    const r = await api.fetchApi("/prompt_palette/categories");
    return await r.json();
  },
  async search(q) {
    const r = await api.fetchApi(`/prompt_palette/search?q=${encodeURIComponent(q)}`);
    return (await r.json()).items || [];
  },
  async preview(name) {
    const r = await api.fetchApi(`/prompt_palette/preview?name=${encodeURIComponent(name)}`);
    return await r.json();
  },
  async content(name) {
    const r = await api.fetchApi(`/prompt_palette/content?name=${encodeURIComponent(name)}`);
    if (!r.ok) return null;
    return await r.json();
  },
  async save(name, content) {
    const r = await api.fetchApi("/prompt_palette/save", {
  ...
  async del(name) {
    const r = await api.fetchApi("/prompt_palette/delete", {
      method: "POST", headers: { "Content-Type": "application/json" },
  ...
  // api.fetchApi() only merges auth/Comfy-User headers on top of whatever
  // `options` provides, so leaving Content-Type unset here still lets the
  // browser set the multipart boundary itself, same as before.
  async setThumbnail(name, file) {
    const fd = new FormData();
    fd.append("name", name);
    fd.append("file", file, file.name);
    const r = await api.fetchApi("/prompt_palette/set_thumbnail", { method: "POST", body: fd });
    return await r.json();
  },
  async removeThumbnail(name) {
    const r = await api.fetchApi("/prompt_palette/remove_thumbnail", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name }),
    });
    return await r.json();
  ...
  async resolve(text, seed, mode) {
    const r = await api.fetchApi("/prompt_palette/resolve", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text, seed, mode }),
    });
    const data = await r.json();
    // token_stats is only present when the backend's CLIP-L tokenizer could
    // be built (see clip_tokenizer.py) - omitted entirely otherwise, so
    // callers can tell "no data" apart from "zero tokens".
    return { resolved: data.resolved || "", tokenStats: data.token_stats || null };
  },
  async refreshWildcards() {
    const r = await api.fetchApi("/prompt_palette/refresh", { method: "POST" });
    if (!r.ok) throw new Error(`refresh failed: ${r.status}`);
    return await r.json();
  },
  async setPath(path) {
    const r = await api.fetchApi("/prompt_palette/set_path", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ path }),
    });
    return await r.json();
  },
  // Count-only sibling of resolve() above, added for the Combinatorial node's
  // live "roughly how many prompts" estimate (see wildcard_combinatorial.js /
  // WildcardResolver.count_combinatorial). Never returns the prompt strings
  // themselves, just how many there'd be and whether that count hit the
  // route's own exploration ceiling.
  async countCombinatorial(text, seed, maxPrompts) {
    const r = await api.fetchApi("/prompt_palette/count_combinatorial", {
      method: "POST", headers: { "Content-Type": "application/json" },
...
// Works around a known ComfyUI dialog-cleanup race (same bug class as
// radix-ui/primitives#3445): the modal locks the page with
// document.body.style.pointerEvents = "none" and is supposed to clear it on
// unmount, but that cleanup can lose the race against other DOM/state churn
// in the same tick and never run — leaving every element that doesn't set
// its own explicit pointer-events:auto (our settingsPopup included)
// permanently unclickable until a page refresh. Clearing the inline style
// ourselves after every dialog call is a no-op when the framework's own
// cleanup already worked, and fixes it when it didn't.
let openDialogCount = 0;
function isDialogOpen() { return openDialogCount > 0; }
function releaseDialogPointerLock() {
...
async function dialogPrompt(opts) {
  openDialogCount++;
  try {
    return await app.extensionManager.dialog.prompt(opts);
  } finally {
    openDialogCount--;
...
async function dialogConfirm(opts) {
  openDialogCount++;
  try {
    return await app.extensionManager.dialog.confirm(opts);
  } finally {
    openDialogCount--;
    releaseDialogPointerLock();
  }
...
// ---------------------------------------------------------------------
// Basic edit actions (Cut / Copy / Paste / Select All), offered alongside
// the wildcard-specific items in both right-click-driven menus below (the
// token/hover inject menu, and the selection ctx menu further down).
// Right-clicking a token or a selection calls e.preventDefault() on the
// textarea's own "contextmenu" event to show one of those menus instead —
// which otherwise silently takes the browser's native Cut/Copy/Paste/Select
// All away with it. These restore that functionality inside whichever menu
// took its place, without removing any of the existing wildcard actions.
// ---------------------------------------------------------------------
...
function runTextareaEditCommand(textarea, render, cmd) {
  textarea.focus();
  if (cmd === "selectAll") { textarea.select(); return; }
  if (cmd === "paste") {
    // Clipboard.readText() needs a secure context + permission and can
    // reject silently in plenty of setups — execCommand("paste") is the
    // fallback, itself blocked in some browsers with nothing further to
    // fall back to at that point (there's no other scriptable way left to
    ...
    (navigator.clipboard && navigator.clipboard.readText
      ...
      // setRangeText mutates .value directly without firing "input", unlike
      // the execCommand path below — dispatch one so the existing textarea
      // "input" listeners (render, autocomplete, etc.) still run.
      textarea.dispatchEvent(new Event("input", { bubbles: true }));
    }).catch(() => {
      try { document.execCommand("paste"); } catch (e) { /* nothing left to fall back to */ }
    });
    return;
  }
  // cut / copy: document.execCommand is deprecated but remains the most
  // broadly supported way to trigger these from a synthetic (non-native)
  // click, and — unlike setRangeText above — already fires a real "input"
  // event on its own when it changes the value (cut), so no extra render
  // call is needed here.
  try { document.execCommand(cmd); } catch (e) { /* no-op */ }
}
...
// Permanently hides a native LiteGraph/ComfyUI widget whose value still
// needs to serialize, while Prompt Palette mirrors that value in its own DOM
// controls. Current Nodes 2.0 builds honor both options.hidden and
// options.canvasOnly; older canvas builds still get the hidden type/no-op draw
// and zero-height computeSize fallback.
function hideNativeWidget(widget) {
  ...
  // Legacy DOM-backed widgets may already be mounted. Hide the widget element
  // and its old wrapper, but never walk or modify the Vue node/grid itself.
  const element = widget.element || widget.inputEl;
  if (element) {
    element.style.setProperty("display", "none", "important");
    const wrapperEl = element.parentElement;
    if (wrapperEl && !wrapperEl.matches('[data-testid="node-widget"]')) {
      wrapperEl.style.setProperty("display", "none", "important");
    }
  }
}

// addDOMWidget can be queried before LiteGraph has assigned widgets_start_y.
// One deferred redraw lets both canvas and Vue modes recompute the widget box
// after the node has completed its first real layout pass.
function scheduleDomWidgetRemeasure(node) {
  requestAnimationFrame(() => {
    requestAnimationFrame(() => node.graph?.setDirtyCanvas(true, true));
  });
}
...
// Nodes 2.0 uses the DOM widget as an `auto` CSS-grid row inside an outer
// node whose configured height is only a CSS minimum. The matching stylesheet
// applies block-size containment to Prompt Palette's own frame as soon as Vue
// mounts it, so a huge category tree cannot become that row's intrinsic
// minimum. These callbacks still tell LiteGraph how much of the current node
// body is available for legacy positioning and resize bookkeeping.

// ---- Shared optional-socket toggle helpers ----
// Drives the "hide this input/output socket on this node instance" settings
// checkboxes for both PromptPaletteEditor (below, the full input+output
// settings section) and PromptPaletteCombinatorial (wildcard_combinatorial.js,
// which only needs an output toggle for its own "prompt" output) — pulled out
// here instead of duplicated so both nodes hide/show sockets through the
// exact same LiteGraph calls and the exact same node.properties shape.
//
// State lives in node.properties.wg_io = { inputs: {}, outputs: {} }, kept
// separate from the global theme/font settings since which sockets a given
// node instance exposes is a per-node choice, not an editor-wide preference.
//
// A def with `default: true` starts ON (visible) for both brand-new nodes
// and any already-saved workflow where this exact key has never been
// toggled before (i.e. the property key is still absent) — that's what lets
// a previously-always-on output (like PromptPaletteEditor's "prompt" and
// "clip_token_count") become toggleable without retroactively vanishing off
// every workflow that already has it wired up. Defs without `default` fall
// back to false/hidden, matching every pre-existing toggle in this file.
...
function syncIoSocket(node, kind, def, enabled) {
  ...
  const idx = ioSocketIndex(list, def.key);
  if (enabled && idx === -1) {
...
// Backfills wg_io for a socket that's already physically present on the live
// node (survived LGraphNode.configure()'s own restore of node.inputs/outputs
// from the saved graph JSON — see LGraphNode.ts, which copies those arrays
// straight from the save regardless of node.properties) but has never had an
// explicit on/off recorded for it. Without this, ioEnabled() falls back to
// def.default — false for every def except prompt/clip_token_count — which
// is only correct for a genuinely brand-new node. For an OLDER saved
// workflow (built before this toggle system existed, or before a given def
// was added — e.g. a Krea-style pipeline wired straight into `conditioning`)
// the socket and its real link are already sitting on node.outputs by the
// time this runs, restored by litegraph itself independently of wg_io. Left
// alone, the very next syncIoSocket() call sees "enabled=false, idx!==-1"
// for that socket and calls removeOutput() on it — silently tearing out a
// link the user never asked to remove, on every single reload, purely
// because the toggle system never recorded an opinion about it.
//
// MUST ONLY be called for a node that has actually gone through
// LGraphNode.configure() this session (see the `node._wgConfigured` guard at
// both call sites below) — a truly fresh, just-created node also has every
// optional socket sitting unstripped on node.outputs for one tick (backend
// declares them all unconditionally; nothing has hidden the non-default ones
// yet), which would look identical to "restored with a pre-existing
// connection" if this ran unconditionally, defeating the whole toggle
// feature for new nodes.
function migrateIoState(node, kind, defs) {
  const bucket = kind === "input" ? ioState(node).inputs : ioState(node).outputs;
  const list = kind === "input" ? node.inputs : node.outputs;
  defs.forEach(def => {
    if (bucket[def.key] === undefined && ioSocketIndex(list, def.key) !== -1) {
      bucket[def.key] = true;
    }
  });
}

// Renders one settings-panel toggle row for `def` into `wrap`, wired up to
// ioState()/syncIoSocket() above. `containerAttr` is "data-io-in" or
// "data-io-out" purely for debugging/inspection — nothing reads it back.
function renderIoToggleRow(node, kind, def, wrap, containerAttr) {
  const row = document.createElement("div");
  row.className = "wg-toggle-row";
  row.title = def.desc;
  row.innerHTML = `<label style="margin:0;">${escapeHtml(def.label)}</label><input type="checkbox" ${containerAttr}="${def.key}">`;
  const cb = row.querySelector("input");
  cb.checked = ioEnabled(node, kind, def);
  cb.addEventListener("change", () => {
    const bucket = kind === "input" ? ioState(node).inputs : ioState(node).outputs;
    bucket[def.key] = cb.checked;
    syncIoSocket(node, kind, def, cb.checked);
  });
...
function buildWildcardWidget(node, hiddenWidget) {
    ...
    <div class="wg-main">
      ...
      <div class="wg-body">
        ...
        <div class="wg-footer">
          ...
          <span class="wg-hint" data-el="tokenCount" data-el-token title="CLIP-L token count of the resolved prompt (visible while Show resolved is on)"></span>
  ...
  // Every socket here is already declared in PromptPaletteEditor.define_schema()
  // (nodes.py) — inputs as optional=True, outputs unconditionally — so the
  // backend always accepts and returns them. What these toggles control is
  // purely whether the socket is *visible* on this node instance, via
  // LiteGraph's own addInput/removeInput/addOutput/removeOutput.
  ...
  const IO_OUTPUT_DEFS = [
    ...
    // These two used to be permanently-on sockets with no toggle at all (every
    // other output above already had one). `default: true` is what makes that
    // safe to change here: an already-saved workflow's node.properties.wg_io
    // has no "prompt"/"clip_token_count" key yet, so ioEnabled() falls back to
    // this default and the socket stays exactly as visible as it always was —
    // only a user who explicitly unchecks it in Settings ever hides it.
    { key: "prompt", type: "STRING", label: "Prompt", default: true, desc: "The final resolved prompt text (or the enhancer override, if that was used). This is this node's primary output \u2014 hide only if you're chaining purely through the other outputs below." },
    { key: "clip_token_count", type: "INT", label: "CLIP token count", default: true, desc: "Real CLIP-L token count of the `prompt` output (start/end/pad tokens not counted). -1 if the tokenizer couldn't be built in this environment." },
  ];

  // ioState/ioEnabled/syncIoSocket/renderIoToggleRow are shared with
  // PromptPaletteCombinatorial's own (smaller) output-toggle section — see
  // their definitions above buildWildcardWidget for the full rationale.
  node.properties = node.properties || {};
  ...
  function renderIoToggles() {
    ...
    inWrap.innerHTML = "";
    IO_INPUT_DEFS.forEach(def => renderIoToggleRow(node, "input", def, inWrap, "data-io-in"));
    outWrap.innerHTML = "";
    IO_OUTPUT_DEFS.forEach(def => renderIoToggleRow(node, "output", def, outWrap, "data-io-out"));
  }
  renderIoToggles();
  // Backend declares all sockets unconditionally, so LiteGraph auto-creates
  // slots for every one of them the moment the node is built. Strip/restore
  // each one to match its current toggle state (falling back to `def.default`
  // when a key has never been toggled) so a brand-new node starts clean with
  // only its default-on sockets, while an already-saved workflow's sockets
  // come back exactly as they were.
  //
  // DEFERRED (setTimeout 0) -- this used to run synchronously right here,
  // which is exactly what was causing sockets to come back disconnected,
  // reordered, or wired to the wrong type after reloading a workflow.
  // onNodeCreated fires while ComfyUI is still constructing this node as
  // part of restoring a saved graph -- BEFORE it copies the real, saved
  // node.properties.wg_io onto this node (that happens inside this node's
  // own configure() call, which runs after onNodeCreated returns; see
  // onConfigure below). So a synchronous call here only ever saw the empty
  // wg_io just initialized a few lines up, never the real saved one, and
  // stripped every non-default-on socket (model, clip, conditioning, ...)
  // down via real addOutput/removeOutput calls -- right as ComfyUI was
  // about to reconnect this node's saved links against those exact slot
  // positions (links are serialized as [node_id, slot_index], see
  // OUTPUT_SLOT_ORDER's comment below). Reconnecting against a socket
  // array that had just been reshuffled out from under it landed each link
  // on whatever ended up at that numeric slot afterwards, not on the socket
  // it was actually saved against -- hence links coming back disconnected,
  // shifted onto a neighboring socket, or landing on the wrong type.
  // Deferring past the current tick (same pattern as
  // reassertHiddenWidgets/_wgRefreshFromHidden below) lets configure()
  // finish restoring both the real wg_io and this node's actual saved
  // inputs/outputs/links first; syncIoSocket then runs once against state
  // that's already correct, so for a restored node every call in this pass
  // becomes a no-op instead of a race. A brand-new node has no saved links
  // to race against, so the one-tick delay changes nothing for it.
  setTimeout(() => {
    // node._wgConfigured (set as the very first line of the wrapped
    // onConfigure below, before anything else runs) is only ever true here
    // if LGraphNode.configure() already ran for this node THIS tick —
    // configure() is fully synchronous and always completes before any
    // setTimeout(0) callback fires, onConfigure included, so the flag is
    // guaranteed to already be settled one way or the other by now. True
    // means "restored from a saved workflow" -> safe to infer any
    // already-present, never-toggled socket as pre-existing rather than
    // freshly-defaulted. Left unset (brand-new node, configure() never
    // called) this deliberately does nothing, so new nodes still start
    // clean at their real defaults. See migrateIoState()'s own comment.
    if (node._wgConfigured) {
  ...
  // Only ever invoked from inside onConfigure (see the deferred call at the
  // bottom of this file), so node._wgConfigured is always true by the time
  // this runs — the check is here anyway so this function stays correct on
  // its own if anything else ever calls it directly.
    ...
    while (i < text.length) {
      ...
      if (wildcardMatch) {
        ...
        if (isKnown(innerName)) {
          ...
          // font-weight here would render wider than the plain, unweighted
          // text in the real (invisible) textarea underneath for any font
          // where bold isn't fixed-width — see the width-drift note by the
          // .wg-tok-* rules in the CSS. text-shadow gives the same "bolder"
          // cue without changing the glyphs' advance width.
          out += `<span class="wg-token" style="color:${color}; text-shadow:0 0 0.35px currentColor;">${escapeHtml(token)}</span>`;
  ...
  function render() {
    ...
    try {
      const result = highlightText(textarea.value);
      names = result.names;
      categoriesInUse = result.categoriesInUse;
      categoryHueMap = result.categoryHueMap;
      // A single trailing "\n" at the very end of a white-space:pre-wrap
      // block doesn't register as an extra visible/measurable line unless
      // something follows it — so only pad when the real text actually
      // ends in a newline (to make that last empty line's height count),
      // never unconditionally, or the overlay ends up permanently one line
      // taller than the real textarea.
      highlight.innerHTML = result.html + (textarea.value.endsWith("\n") ? "\n" : "");
    } catch (e) {
      // If highlighting ever throws on some input, don't leave `highlight`
      // frozen on the PREVIOUS render's HTML while textarea.value has
      // already moved on — that divergence is exactly what makes clicks
      // and the caret look like they're landing in the wrong spot, even
      // though the real (invisible) textarea's own cursor was never
      // actually wrong. Falling back to plain, unhighlighted text keeps
      // the overlay character-accurate (just uncolored) until this is
      // fixed, instead of silently stale.
      console.error("[wildcard_editor] highlightText failed — falling back to plain text:", e);
      highlight.textContent = textarea.value + (textarea.value.endsWith("\n") ? "\n" : "");
    }
    legend.innerHTML = "";
  ...
  // Renders the CLIP-L token badge from a /resolve response's token_stats
  // (see clip_tokenizer.py). Null clears it -- used both when the backend
  // couldn't build a tokenizer and when the resolved view is toggled off,
  // so a stale count is never left showing next to raw (unresolved) text.
  function updateTokenBadge(stats) {
    tokenCountEl.classList.remove("wg-token-warn", "wg-token-danger");
    if (!stats) {
      tokenCountEl.textContent = "";
      tokenCountEl.title = "CLIP-L token count of the resolved prompt (visible while Show resolved is on)";
      return;
    }
  ...
  async function refreshResolvedView() {
    ...
    try {
      const { resolved, tokenStats } = await API.resolve(textarea.value, seed, mode);
      resolvedView.textContent = resolved;
      updateTokenBadge(tokenStats);
    } catch (e) {
  ...
  async function promptSaveRecipe(paths) {
    ...
    const raw = await dialogPrompt({
      ...
      message: `Name this Palette Recipe (combines ${unique.length} wildcard${unique.length === 1 ? "" : "s"}):`,
      defaultValue: "",
    });
    if (raw === null || raw === undefined) return false; // cancelled
    ...
    if (knownSet.has(fullName)) {
      const overwrite = await dialogConfirm({
        title: "Overwrite Recipe",
        message: `"${fullName}" already exists \u2014 overwrite it?`,
      });
  ...
  async function promptExtractSelectionToWildcard(start, end) {
    ...
    if (!trimmed) return;
    const raw = await dialogPrompt({
      title: "Extract to New Wildcard",
      message: 'Save selection as new wildcard (e.g. "scenes/sunset_girl"):',
      defaultValue: "",
    });
  ...
  const handleOutsideClick = (e) => {
    ...
    // renders outside settingsPopup's DOM subtree, so clicking into or
    // confirming it counts as an "outside" click here too — without this
    // guard it would close the settings panel out from under the dialog
  ...
  function applyFontSettings() {
    ...
    // A font-family/size change can change the textarea's content height
    // and silently clamp its scrollTop/scrollLeft, but that doesn't fire a
    // native "scroll" event — and the highlight overlay only ever re-syncs
    // off that event (see the textarea "scroll" listener above). Without
    // this, the highlight layer can be left showing a stale scroll offset
    // relative to the real textarea after any font change, even a revert
    // back to the default font, until the user happens to scroll manually.
    highlight.scrollTop = textarea.scrollTop;
    highlight.scrollLeft = textarea.scrollLeft;
  }
  ...
  // Each of these touches its own slice of the settings panel; none of them
  // are supposed to be able to fail on a normal init, but if one ever does
  // (e.g. a template/id mismatch), letting it throw here used to silently
  // abort every line after it — including the initial render() call below.
  // That leaves `highlight` permanently empty/stale while the real
  // (transparent-text) `textarea` keeps working perfectly normally: the
  // user sees stale/blank overlay text but clicks and typing still land at
  // the *real*, correct position in the invisible textarea underneath —
  // which reads exactly like "clicks and the caret land in the wrong spot"
  // even though the caret itself was never wrong. try/catch here turns a
  // failure into a loud console error instead of that silent, confusing
  // divergence, and guarantees it can never block the render() that
  // actually syncs the overlay to the real text.
  try {
    applyUiTheme();
    refreshUiThemeUI();
    applyFontSettings();
    ...
    console.error("[wildcard_editor] theme/font/toolbar init failed — overlay may be desynced from the real textarea until this is fixed:", e);
  }
  ...
  //
  // Two things about that ring have to be enforced here in JS, not left to
  // wildcard_editor.css, or the ring stops being "reachable" and instead
  // becomes an invisible dead zone stuck to wherever the node currently
  // ends:
  //
  // 1. box-sizing. ComfyUI measures/positions this element's OUTER box
  //    (the one addDOMWidget's getHeight() above reports, at the node's
  //    real width) and never re-measures it after paint. Any CSS padding
  //    on .wg-node-frame under the browser's default content-box model
  //    is added ON TOP of that reported size, so the frame's real,
  //    interactive footprint silently overshoots the box ComfyUI thinks
  //    it occupies — by exactly the padding amount, on every side. With
  //    ComfyUI's "Enable DOM element clipping" setting off (the common
  //    case — many custom nodes need it off so their own popups/dropdowns
  //    aren't clipped), that overshoot isn't clipped away either: it's a
  //    real, unclipped chunk of page sitting past the node's true screen
  //    edge. border-box folds the padding back inside the reported size
  //    instead of adding to it, so the frame's actual footprint always
  //    matches the box ComfyUI positioned it at — no overshoot, on any
  //    side, regardless of how much padding the stylesheet ends up using.
  //
  // 2. pointer-events. Even with (1) fixed, the padding ring itself is
  //    still a real part of the frame element, and every element is
  //    pointer-events:auto by default — so the ring keeps swallowing
  //    clicks meant for the canvas underneath it (the resize handle, the
  //    right-click menu, drag-select) unless something tells the browser
  //    to look past it. `none` on the frame makes the browser skip it
  //    entirely and hit-test whatever's actually underneath for any point
  //    inside the ring; `auto` on `root` re-enables real interaction for
  //    the UI it actually wraps, since pointer-events:none does NOT
  //    inherit past an element that explicitly overrides it back to auto.
...
// order the user happened to click the checkboxes in," not the fixed order
// PromptPaletteEditor.define_schema() declares in nodes.py.
//
...
// though — io.NodeOutput's positional values are matched to the schema's
// fixed outputs list. So the moment visible order and schema order diverge,
// a link recorded with the visible index silently pulls whatever value
// happens to sit at that position in the backend output tuple instead of the
// one the user actually wired up.
//
...
// "position in the live node.outputs array" to "position in the schema's
// fixed outputs list." This only has to touch links; widget values serialize
// fine as-is.

// MUST exactly match the PromptPaletteEditor Python node's outputs list in
// nodes.py define_schema(), in order — index here = the backend's real,
// fixed output position. If that outputs list ever changes, update this to
// match. Every entry here is user-toggleable via IO_OUTPUT_DEFS (including
// "prompt" and "clip_token_count", which default to on but can be hidden
// like any other), which is exactly why visible order can diverge from this
// fixed order and needs remapping in the first place. A toggled-off output
// simply can't have a live link to remap, so nothing here needs to change
// based on which sockets happen to be hidden on a given node instance.
const OUTPUT_SLOT_ORDER = [
  ...
  "wildcards_used_count", "used_enhancer", "clip_token_count",
];
...
function remapPromptPaletteOutputs(promptResult) {
  ...
  for (const nodeId in output) {
    ...
    for (const inputName in inputs) {
      ...
      // rootGraph is the current frontend API; app.graph is retained as a
      // compatibility fallback for older bundled frontends.
      const graph = app.rootGraph || app.graph;
      const liveNode = graph?.getNodeById(sourceId) || graph?.getNodeById(Number(sourceId));
...
app.registerExtension({
  ...
  // initializing) rather than nodeCreated() (fired for every node instance).
  ...
  async nodeCreated(node) {
    ...
    const { root: container, refreshFromHidden, reassertHiddenWidgets, cleanup } = buildWildcardWidget(node, hiddenWidget);
    let domWidget;
    domWidget = node.addDOMWidget("prompt_palette_ui", "div", container, {
      ...
      // The hidden text widget owns serialization; this visual DOM widget must
      // never add another positional widgets_values entry.
      serialize: false,
      // The node-level MIN_HEIGHT below is the minimum for the WHOLE node.
      // Giving the widget that same minimum makes Nodes 2.0 add title/slots/
      // padding on top of it and turns the requested height into a content
      // floor. Let the widget shrink inside the node and cap it to the space
      // LiteGraph actually allocated for this row.
      getMinHeight: () => 0,
    ...
    // ComfyUI mounts the real DOM element for each hidden native widget
    // (hiddenWidget/seedWidget/controlWidget/modeWidget) as part of its
    // own reactive render pass, which runs AFTER this onNodeCreated call
    // returns — not during it. That mount stomps the inline
    // "display: none !important" hideNativeWidget() just set a few lines
    // up inside buildWildcardWidget(), because ComfyUI writes that
    // element's whole style attribute fresh when it mounts/re-renders it,
    // wiping out anything set before that point. Left alone, the result
    // is ComfyUI's own ".dom-widget" wrapper for that native widget
    // sitting on top of the canvas — position:fixed, pointer-events:auto,
    // no display:none — invisible but still eating every click at the
    // node's location.
    //
    // onConfigure below already re-runs reassertHiddenWidgets() (deferred
    // the same way) to fix this exact race for nodes loaded from a saved
    // workflow / restored on a page refresh — which is why reloading the
    // page "fixes" existing nodes. But onNodeCreated never called it, so
    // a brand-new node had no second pass and stayed broken until the
    // *next* reload ran it through onConfigure instead. Deferring the
    // same call here closes that gap for fresh nodes too.
    ...
    node.onConfigure = function () {
      // Set first, before anything else (including the original onConfigure)
      // runs — this is the signal migrateIoState's callers gate on to tell
      // "restored from a saved workflow" apart from "brand-new node that
      // still happens to have its full unfiltered socket set for one tick."
      // See migrateIoState()'s comment for why that distinction matters.
      ...
      // ComfyUI's own configure/restore step (this ran via onConfigure.apply
      // above, before this comment's code runs) recreates/re-touches widget
      // DOM elements, which undoes the display:none we forced on the hidden
      // native widgets' wrapper divs back in onNodeCreated -- that's what
      // left an invisible click-blocking box below nodes that were loaded
      // from a saved workflow (as opposed to freshly created ones, which
      // never go through this path). Re-hide them here, after the restore
      // that undid it has finished.
      if (this._wgReassertHiddenWidgets) setTimeout(() => this._wgReassertHiddenWidgets(), 0);
      // Same "invisible click-blocking strip below the node" bug as
      // onNodeCreated above, for the path that actually reproduces it —
      // see scheduleDomWidgetRemeasure()'s comment.
...
// ---------------------------------------------------------------------
// Additive exports for companion Prompt Palette node frontends (currently:
// wildcard_combinatorial.js) that reuse this file's API client, "__"
// autocomplete engine, folder-picker persistence, and color/text utilities
// rather than reimplementing plainer versions of them. Nothing above this
// line changes behavior -- these are the same bindings already used
// throughout this file, just also handed to importers. acState is exported
// live (read-only from the outside) purely so a companion node's own
// cleanup can check "is the shared autocomplete menu currently open against
// MY textarea" the same way this file's own cleanup does.
// ---------------------------------------------------------------------
export {
  API, loadTheme, saveTheme, defaultTheme, escapeHtml, highlightMatch, categoryOf, hashStr, hideNativeWidget,
  editorStylesReady,
  ...
  getDomWidgetAvailableHeight, scheduleDomWidgetRemeasure,
  ...
  ioState, ioEnabled, syncIoSocket, migrateIoState, renderIoToggleRow,
  ...
  dialogPrompt, dialogConfirm, isDialogOpen,
};